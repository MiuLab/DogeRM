...
def get_args():
    ...
    parser.add_argument(
        '--vhead_path', type=str, default=None
    )
    parser.add_argument(
        "--to_wandb", action="store_true"
    )
    parser.add_argument(
        "--run_name", type=str, default=None
    )
    parser.add_argument(
        "--proj_name", type=str, default=None
    )
    parser.add_argument(
        "--seq_weight", type=float, default=0.5
    )
    parser.add_argument(
        "--lm_weight", type=float, default=0.5
    )
    parser.add_argument(
        "--collect_reward", action="store_true"
    )
...
def main():
    ...
    dataset, subsets = load_eval_dataset(
        ...
        use_system_prompt=("mistral" not in args.model)
    )
    ...
    if args.debug:
        dataset = dataset.select(range(13))
        subsets = subsets[:13]
        ids = ids[:13]
        for data in dataset:
            print(data)
        # exit(0)
    if args.collect_reward:
        idx = [i for i in range(len(subsets)) if "hep" in subsets[i]]
    ...
    if quantized:
        ...
        model_kwargs = {"device_map": {"": current_device}, "torch_dtype": torch.float16}
    if args.vhead_path is not None:
        model_kwargs.update({'head_path': args.vhead_path})
    ...
    if args.to_wandb:
        upload_dict = {k : v for k, v in results_grouped.items() if not isinstance(v, str)}
        run.log(upload_dict)
